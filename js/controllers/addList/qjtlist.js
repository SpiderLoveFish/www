...
alert(JSON.stringify(data))
	common.postApi('GetQJT ', data, function(response) {
		...
		for (var i = 0; i < dataArray[0].length; i++) {
			...
			list.innerHTML += html_CanYu.replace('@ID', obj.desid).replace('@STheme', obj.DyGraphicsName).replace('@SContext', obj.remarks).replace('@ReleaseTime', obj.dotime).replace('@flag', obj.lx).replace('@url', obj.DyUrl); 
