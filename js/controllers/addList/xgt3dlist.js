...
alert(JSON.stringify(data))
	common.postApi('GetQJT ', data, function(response) {
		...
		for (var i = 0; i < dataArray[0].length; i++) {
			...
			list.innerHTML += html_CanYu.replace('@ID', obj.id).replace('@STheme', obj.roomTypeName).replace('@SContext', obj.Remarks).replace('@ReleaseTime', obj.DoTime).replace('@flag', obj.lx).replace('@url', obj.img); 
