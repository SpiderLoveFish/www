...
mui.plusReady(function() {
	...
	mui.plusReady(function() {
		...
		function transDataHandler(event) {
			...
			var template = "<div class='responsible_person_xx r'><p>@name</p></div>";
		...
		function insertLeave() {
			...
			var leaveTitle = common.textValiAlert(document.getElementById("leaveTitle").value, "请填写标题");
			if (!leaveTitle) {
				return;
			}
			var leaveContent = common.textValiAlert(document.getElementById("leaveContent").value, "请填写审批内容");
			if (!leaveContent) {
				return;
			}
			//负责人逻辑判断
			...
			var type = "insert";
			//推送消息后跳转 参与未审批
			var argu = "{'vid':'generalapprove','pid':'1'}"; //推送接收参数
			var pushTitle = getUserInfo().UserName + "的通用申请:" + '通用审批标题';
			//图片上传
